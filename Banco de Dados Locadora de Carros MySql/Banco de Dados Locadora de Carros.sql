...
select v.modelo, v.marca, count(lv.idlocacao) as QuantidadeLocacoes from veiculo v join locacaoveiculo LV on v.idveiculo = LV.idveiculo group by v.idveiculo order by QuantidadeLocacoes desc;

select C.idCliente, C.nome, sum(p.valortotal) as ValorDevido from Cliente C join locacao L on c.idcliente = L.idcliente join pagamento P on L.idpagamento = P.idpagamento where P.estado = 'Pendente' group by C.idCliente, C.nome order by C.nome asc;